...
function rankMeta(i) {
  if (i === 0) return { badge: "🏆 Leader", line: "Keep the lead — clear approvals fast." };
  if (i === 1) return { badge: "🥈 Chasing", line: "One strong finish can flip the table." };
  return { badge: "🔥 Comeback Mode", line: "Unblock + clear overdue = fastest climb." };
}
...
export default function Landing() {
  ...
  const topLine = useMemo(() => {
    ...
    if (gap <= 5) return "Tight race today. One approval can change the leader.";
    ...
    <div className="min-h-full bg-slate-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-7 space-y-5">
        ...
        <div className="rounded-3xl bg-gradient-to-br from-slate-900 to-slate-700 text-white p-6 shadow-sm border border-white/10">
          ...
          <div className="text-2xl font-semibold mt-1">Win the week: ship projects, clear approvals.</div>
                ...
                <tbody>
                  {stats.map((h, i) => (
                    <tr key={h.head} className="border-t border-slate-100">
                      <td className="py-3 pr-3 font-semibold text-slate-900">#{i + 1}</td>
        ...
        <Card className="p-5">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <div className="text-sm text-slate-600">Filter and compare workload instantly.</div>
            </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((p) => {
                  ...
                  const overdue = (p.approvals || []).some((a) => a.state === "Pending" && isOverdue(a.dueDate));
                  return (
                    <tr key={p.id} className="border-t border-slate-100">
                      ...
                      <td className="py-3 pr-3"><Chip>{p.vendor}</Chip></td>
                      <td className="py-3 pr-3">
                      ...
                      <td className={`py-3 pr-3 ${p.status !== "Done" && isOverdue(p.targetDate) ? "text-rose-700 font-medium" : ""}`}>
                        {fmt(p.targetDate)}
                      </td>
                      <td className={`py-3 pr-3 ${overdue ? "text-rose-700 font-semibold" : "text-slate-900 font-medium"}`}>
                        {pending}
                      </td>
                    </tr>
                  );
        ...
        {/* Vendor heatmap */}
        <Card className="p-5">
          <div className="text-lg font-semibold text-slate-900">Vendor Heatmap</div>
          <div className="text-sm text-slate-600 mt-1">Totals, blocked, and done by vendor.</div>
              ...
              <tbody>
                {VENDORS.map((v) => (
                  <tr key={v} className="border-t border-slate-100">
                    <td className="py-3 pr-3 font-medium text-slate-900">{v}</td>
                    <td className="py-3 pr-3">{vendorHeat[v]?.total ?? 0}</td>
                    <td className={`py-3 pr-3 ${(vendorHeat[v]?.blocked ?? 0) > 0 ? "text-rose-700 font-semibold" : ""}`}>
                      {vendorHeat[v]?.blocked ?? 0}
                    </td>
                    <td className={`py-3 pr-3 ${(vendorHeat[v]?.done ?? 0) > 0 ? "text-emerald-700 font-semibold" : ""}`}>
                      {vendorHeat[v]?.done ?? 0}
                    </td>
                  </tr>
                ))}
        ...
        <div className="space-y-2">
          ...
          <div className="text-sm text-slate-600">A single glance view of delivery dates and overlap.</div>
